//this program is the first Microbit for the game Battleship
...
radio.onReceivedNumber(function (receivedNumber) {
    if (receivedNumber == 15) {
        ...
        y2 = 0
        led.plot(x2, y2)
//If this microbit receives the number 15, it sets Turn to 2
//and plots x2,y2 at 0,0. This allows the microbit to enable
//the functions that show it is microbit 1's turn to go.
        ...
        HOLD = 0
//If they recieve the number 35, HOLD will be changed to 1
...
//If they receive 55, HOLD turns to 1, Stop will turn to 1
...
//from the second microbit and the same for the other two.
function MoveHo () {
    if (x2 == 4) {
        x2 = -1
    }
...
//This saves the x coordinate and the y coordinate of the attack 
//in an empty string
function HitAction () {
    basic.showLeds(`
        . . . . .
...
//'Hit', then the radio Number was set to 30
function Check () {
    CheckMiss = 0
...
//boat. First, if the positionboat12, or the second user's 
...
//send the number 40 and add it to the string. After all this set Hold to 0, Turn to 
//3, x2 to 0, y2 to 0 and send the number 10. This function is important as it is 
...
//set Turn to 2 and showing 'Go' and plotting the first point
    }
    if (Turn == 0) {
...
//If you have not hit the whole thrid boat plot it at 75% brightness 
